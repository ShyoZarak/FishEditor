...
public class Track : MonoBehaviour
{
	...
	public List<int> UnitPerStep = new List<int>();
	//public bool IsCurved;
	public int LinesCount = 25;
	public Color Color = Color.white;
	//public bool IsClosed;
	public bool IsShowLinePoints = true;
	...
	public List<Vector3> GetPoints()
	{
		...
		//if (IsCurved)
		{
			for (int ind = 0; ind < Waypoints.Count; ind++)
			{
				...
				float length = BezierApproximation.GetLength(
					Waypoints[ind].position, GetHandle1(ind),
					Waypoints[nextInd].position, GetHandle2(nextInd), SampleCount);


				float unityPerStep = ind < UnitPerStep.Count ? UnitPerStep[ind] : 2f;
				int count = Mathf.CeilToInt(length / unityPerStep);

				List<Vector3> lines = BezierApproximation.GetLines(
					Waypoints[ind].position, GetHandle1(ind),
					Waypoints[nextInd].position, GetHandle2(nextInd), count);

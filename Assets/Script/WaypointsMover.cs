...
public class WaypointsMover : MonoBehaviour
{
	...
	{
		return _targetIndex == Path.WaypointsCount - 1;
	}
	...
	protected IEnumerator OnWaypointReached()
	{
		...
		_pathPartStartTime = Time.time;
		
		if (LoopType == LoopType.Looped && _targetIndex == 0)
			StartCoroutine(OnWaypointReached());
	...
	public void Go(Track path, int startWaypointIndex)
	{
		Path = path;
	...
	public void Go(Track path)
	{
		Go(path, 0);
	}
	
	public void Go(Track path, Transform startWaypoint)
	{
